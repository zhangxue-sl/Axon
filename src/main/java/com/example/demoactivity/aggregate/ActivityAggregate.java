...
public class ActivityAggregate {
    ...
    public ActivityAggregate(CreateActivityCommand comm) {
        AggregateLifecycle.apply(new ActivityCreateEvent(comm.getId(), comm.getName(), comm.getStartDate(),
                comm.getContent(), comm.getCount(), comm.getPalceId()));
        
    }

    @CommandHandler
    public void handle(UpdateActivityCommand comm) {
        AggregateLifecycle.apply(new ActivityUpdateEvent(comm.getId(), comm.getName(), comm.getStartDate(),
                comm.getContent(), comm.getCount(), comm.getPalceId()));
